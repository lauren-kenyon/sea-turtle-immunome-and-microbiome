...
data_drive_with_GO <- "C:\\Users\\laure\\Documents\\Thesis\\Annotations\\GO_lists\\antiviral_GO_list_of_immunome_"
...
GT_fp <- paste(data_drive_with_GO, "Cm", annot_name, sep = "")
...
Dc_Cm <- uniq_shared(list(Dc_ls, GT_ls),list(Ei_ls, Cc_ls) )
dc_cm_table <-  get_GO_table(Dc_Cm) 
fp_for_dc_cm_table <- "C:\\Users\\laure\\Documents\\Thesis\\antiviral_Dc_Cm.xls"
write.table(dc_cm_table, fp_for_dc_cm_table, row.names = FALSE, col.names = TRUE, quote = FALSE, sep = "\t")

not_in_Dc <- uniq_shared (list(Cc_ls, GT_ls, Ei_ls), list (Dc_ls))
n_Dc <- get_GO_table(not_in_Dc)
fp_for_not_Dc <- "C:\\Users\\laure\\Documents\\Thesis\\antiviral_not_in_Dc.xls"
write.table(n_Dc, fp_for_not_Dc, row.names = FALSE, col.names = TRUE, quote = FALSE, sep = "\t")

a <- get_GO_table(all_antiviral_GO_terms)
fp_for_ant <- "C:\\Users\\laure\\Documents\\Thesis\\my_antiviral_table.xls"
write.table(a, fp_for_ant, row.names = FALSE, col.names = TRUE, quote = FALSE, sep = "\t")

all_shared <- get_GO_table(uniq_shared(list(Ei_ls, Cc_ls, Dc_ls, GT_ls), list()))
fp_for_shared_antiv <- "C:\\Users\\laure\\Documents\\Thesis\\my_shared_antiviral_table.xls"
write.table(all_shared, fp_for_shared_antiv, row.names = FALSE, col.names = TRUE, quote = FALSE, sep = "\t")
